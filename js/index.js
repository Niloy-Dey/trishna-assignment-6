const displayShows = async () => {
    ...
    allDataDetails(data.data.tools);
}
displayShows();
...
const allDisplayDetails = cards => {
    ...
    cards.forEach(tool => {
        // console.log(tool)
        const {name, image, features, id} = tool;
...
const showSingleData = async (id) => {
    ...
    // const { name, image, description, features,  } = singleId;

    
    
    const title = document.getElementById('exampleModalLabel').innerText = `${singleId.name}`;
    const MoImage = document.getElementById('modal-image').src = `${singleId.image}`;
